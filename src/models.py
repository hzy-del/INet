...
########################################################################
    ...
    def process(self, images, masks):
        ...
        gen_style_loss = self.style_loss(outputs * masks, images * masks) + self.style_loss(outputs, images)
        gen_style_loss = gen_style_loss * self.config.STYLE_LOSS_WEIGHT
        gen_loss += gen_style_loss

        # tv loss
        gen_input_fake = outputs
        gen_input_real = images
        comp_B = gen_input_fake * (1 - masks) + gen_input_real  * masks
        tv_loss = self.TV_loss(comp_B * (1 - masks))
        tv_loss = tv_loss * self.config.TV_LOSS_WEIGHT
        gen_loss += tv_loss
